...
int	calculate_color(int iteration, int max_iterations)
{
	...
	rand_state = seed;
	red = (1101 * rand_state + 12345) % 256;
	rand_state = 1101 * rand_state + 12345;
	green = (1101 * rand_state + 12345) % 256;
	rand_state = 1101 * rand_state + 12345;
	blue = (1101 * rand_state + 12345) % 256;
...
int	main(int ac, char **av)
{
	...
	if ((ac == 2 && !ft_strncmp(av[1], "triangle", 8)) || (ac == 2
			&& !ft_strncmp(av[1], "mandelbrot", 10)) || ((ac == 4 || ac == 2)
	...
	{
		ft_putendl_fd("please enter a correct value like '-0.04
				+0.24' (real & imaginary)", 2), exit(EXIT_FAILURE);
		exit(1);
	}
}
